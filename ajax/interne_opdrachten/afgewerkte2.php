...
if( $_SESSION["tcc_user"]->group_id == 1 )
{
    ...
    foreach( $list_arr as $id => $user )
    {
        ...
        echo "<option ". $sel ." value='".$id."'>".$user."</option>";
    }
...
if( mysqli_num_rows($q_zoek) > 0 )
{
    ...
    while( $opdracht = mysqli_fetch_object($q_zoek) )
    {
        ...
        if( $_GET["user"] != "all" )
        {
            $aantal_open = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM kal_opdrachten_users WHERE opdracht_id = " . $opdracht->id . " AND user_id = ". $_GET["user"] ." AND status = 'done'"));    
        }else
        {
        ...
        if( $aantal_open > 0 )
        {
            ...
            */
            ...
            if( $_SESSION["tcc_user"]->user_id == $opdracht->van_user_id )
            {
                ...
                echo "<input onclick='return confirm(\"Delete this assignment?\")' type='image' src='images/delete.png' name='delete_opdracht' id='delete_opdracht' width='20' height='20' value='". $opdracht->id ."' />";
                echo "</form>";
            }
