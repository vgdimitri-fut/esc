...
if (isset($_POST['btn_transactie']) && !isset($_POST['update'])) {
    $q = "INSERT INTO tbl_transacties (prod_id,status,soort,soort_id,datum,prijs_excl,prijs_incl,btw) VALUES (" . $_POST['product'] . ",'" . $_POST['status'] . "','0'," . $_POST['cus_id1'] . ",'" . changeDate2EU($_POST["datum"]) . "','" . $_POST['prijs_exclu'] . "', '" . $_POST['prijs_inclu'] . "','" . $_POST['btw'] . "')"; 
    mysqli_query($conn, $q);
}
// UPDATE TRANSACTION
if (isset($_POST['btn_transactie']) && isset($_POST['update'])) {
    mysqli_query($conn, "UPDATE tbl_transacties SET prod_id=" . $_POST['product'] . ","
            . " status='" . $_POST['status'] . "',"
            . "soort='0',soort_id=" . $_POST['cus_id1'] . ","
...
//     make map for user and go
    if (!is_dir($_POST['cus_id1']) || !file_exists($_POST['cus_id1'])) {
        @mkdir($_POST['cus_id1'], 0777);
    }
    chdir($_POST['cus_id1']);
...
//                if( $(this).val() == 0 ){
//                    $.post("klanten_ajax.php",{status: 'aankoop'}, function(data){
//                            $('#product').html(data);
//                    });
//                }else{
//                    $.post("klanten_ajax.php",{status: 'verkoop'},function(data){
//                            $('#product').html(data);
//                    });
//                }
//            })
...
if (isset($_GET['trans_id'])) {
    ...
    $q = mysqli_query($conn, "SELECT * FROM tbl_transacties WHERE id=" . $transactie_get_id);
    $get_transactie = mysqli_fetch_object($q);
    $datum = $get_transactie->datum;
    $prijs_incl = $get_transactie->prijs_incl;
    $prijs_excl = $get_transactie->prijs_excl;
    
    echo "<h3>Transactie aanpassen</h3>";
} else {
    $datum = '';
    $prijs_incl = '';
    $prijs_excl = '';
    
    echo "<h3>Transactie toevoegen</h3>";
}
//var_dump($_FILES);
...
if (isset($_GET['trans_id']) )
{
    // opzoeken van het juiste product
    $q = mysqli_query($conn_car, "SELECT * FROM tbl_products WHERE id = " . $get_transactie->prod_id );
...
echo "<tr>";
echo "<td>";
echo "<label>Aan/Verkoop document: </label>";
echo "</td>";
echo "<td>";
...
echo "<input type='hidden' name='cus_id1' value='" . $_GET["klantid"] . "' />";
echo "<input type='submit' name='btn_transactie' id='btn_transactie' value='Opslaan'/>";
echo "</td>";
echo "</tr>";
echo "</table>";
echo "</form>";
