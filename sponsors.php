...
VALUES(". $_POST["lev_id"] .",'kal_sponsors', '". str_replace(" ", "", str_replace(".", "",$_POST["reknr"]) )  ."','". str_replace(" ", "", $_POST["bic"]) ."')";
    ...
    if( !empty( $_FILES["doc"]["name"] ) )
    {
        ...
        for($i=0;$i<$aantal_bestanden;$i++)
        {
            ...
            $q_ins = "INSERT INTO kal_customers_files(cf_cus_id,cf_soort,cf_file) 
                                               VALUES(". $_POST["lev_id"] .",'sponsor_docs','".$_FILES["doc"]["name"][$i]."')";
    ...
    foreach( $_POST as $key => $post )
    {
    	if( substr($key, 0, 11) == "levdoc_del_" )
    	{
            // opzoeken record
            $id = substr( $key, 11 );
            $distri_off = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM kal_customers_files WHERE cf_soort = 'sponsor_docs' AND cf_id = " . $id));
            
            // bestand verwijderen 
            $bestand = "sponsor_docs/" . $_POST["lev_id"] . "/doc/" . $distri_off->cf_file;
            if(file_exists($bestand))
            {
                if(unlink( "sponsor_docs/" . $_POST["lev_id"] . "/doc/" . $distri_off->cf_file ))
                {
                    // record verwijderen
...
if( isset( $_POST["bewaar"] ) && $_POST["bewaar"] == "Bewaar" )
{
    if( !empty( $_POST["n_naam"] ) )
    {
        ...
        $q_ins = "INSERT INTO kal_sponsors(naam,straat,postcode,gemeente,land,email,betalingstermijn,taal,btwperc,btwnr,tel,gsm) 
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        if( $row > 3 )
        {
            ...
            for ($c=0; $c < $num; $c++) {
                ...
                $q_ins = "INSERT INTO kal_sponsors(naam,
                                                       straat,
                                                       postcode,
                                                       gemeente,
                                                       land,
...
<title>Sponsors<?php include "inc/erp_titel.php" ?></title>
<style type='text/css'>
...
$().ready(function() {
	$("#klant").autocomplete("lev_sponsor.php", {
		width: 260,
		matchContains: false,
		mustMatch: false,
		//minChars: 0,
...
{
    $lev = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM kal_sponsors WHERE id = " . $_POST["cus_id1"]));
...
Nieuwe sponsor toevoegen.<br/><br/>
...
if( isset( $_POST["bewaar"] ) && $_POST["bewaar"] == "Bewaar" )
{
    if( empty( $_POST["n_naam"] ) )
    {
        echo "<span class='error'>De naam van de sponsor is verplicht.</span>";
    }
}
...
function searchKlant()
{
    ...
    document.frm_lev.action = "sponsors.php?tab_id=1&klant_id=" + klant;
    document.frm_lev.submit();
}        
</script>    
    <form autocomplete="off" method='post' id='frm_lev' name='frm_lev' action="sponsors.php">
		<label for='klant'>Zoek sponsor :</label> <input type="text" name="klant" id="klant" /> 
    ...
    */
    ...
    if( $lev_id != 0 )
    {
        $lev = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM kal_sponsors WHERE id = " . $lev_id));
        
        if( !$lev )
        {
            die("Sponsor niet gevonden.");
        }
        ...
        */
        ...
        echo "<table>";
        echo "<tr><td><b>Sponsor : </b></td><td><input type='text' name='lev' id='lev' size='40' value='".$lev->naam."' /></td></tr>";
        ...
        $q_zoek = mysqli_query($conn, "SELECT * FROM kal_reknr WHERE tabel = 'kal_sponsors' AND klant_id = " . $lev->id);
        ...
        while( $reknr = mysqli_fetch_object($q_zoek) )
        {
            ...
            */
            ...
            echo "<a onclick=\"javascript:return confirm('Rek.nr. verwijderen?')\" href='sponsors.php?tab_id=1&klant_id=".$lev->id."&actie=delete&rekid=". $reknr->id ."'><img src='images/delete.png' border='0' /></a>";
...
//                                    echo $q;
        ...
        if( mysqli_num_rows($q_zoek_levdocs) > 0 )
		{
			...
			{
				if( file_exists( "sponsor_docs/" . $lev->id . "/doc/" . $lev_docs->cf_file ) )
				{
					...
					echo "</td><td align='left'>";
						...
						echo "<a href='sponsor_docs/" . $lev->id . "/doc/" . $lev_docs->cf_file . "' target='_blank' >";
...
$q_klanten = mysqli_query($conn, "SELECT * FROM kal_sponsors " . $sorteer);

$sorteer = "";

?>
<form name='frm_overzicht' id='frm_overzicht' method='post' action="sponsors.php" >
	<input type='hidden' name='tab_id' id='tab_id' value='1' /> 
